      ...
      <div className="grid grid-cols-1 gap-10 md:grid-cols-2 lg:grid-cols-3">
        {projects.map((project) => (
          <Link href={project.link} key={project.title} target="_blank" rel="noopener noreferrer" className="block h-full">
            <div className="group flex h-full flex-col rounded-[15px] border border-border-gray bg-main-gray shadow-[0_0_15px_var(--glow-color)]  transition-transform duration-300 hover:bg-accent-3 hover:-translate-y-2 sm:w-auto hover:shadow-[0_0_0_var(--glow-color)]">
              ...
              <div className="flex flex-grow flex-col p-4">
                <h3 className="mb-2 text-xl font-semibold text-text-light group-hover:text-black lg:text-2xl">{project.title} <span className="text-text-dark group-hover:text-black">| {project.year}</span></h3>
                <p className="flex-grow text-text-dark group-hover:text-black">{project.description}</p>
              </div>
            </div>
          </Link>
        ))}
